...
#include"Print.h"
#include"Print.cpp"        //Реалтзации шаблонных функций в обязательном порядке подключаются на место вызова 
...
template<typename T>void ShiftLeft(T arr[], const int n, const int number_of_shifts)
{
	for (T i = 0; i < number_of_shifts; i++)
	{
		T buffer = arr[0];
		for (int i = 1; i < n; i++)
		{
